...
def _extract_finding(text: str, section_header: str) -> str:
    """
    Extract the bracketed finding label that follows a section header.
    ...
    text:
        The raw LLM output.
    section_header:
        Regex-safe string matching the bold section title
        (e.g. ``"1. EXISTING ALIGNMENT"``).

    Returns
    -------
    str
        The extracted finding label, or an empty string if not found.
    """
    pattern = rf"\*\*{re.escape(section_header)}\*\*.*?\[Finding:\s*([^\]]+)\]"
...
def _extract_citations(text: str) -> list[str]:
    ...
    list[str]
        ...
        occurrence.
    """
    pattern = r"CAP\s+(?:Goal|Strategy|Section|p\.?|Page|Action)?\s*[A-Z0-9][A-Z0-9\-\.]+"
...
def run_stage1(
    ...
    try:
        ...
        # Extract implementation pathway section
        pathway_match = re.search(
            r"\*\*5\. IMPLEMENTATION PATHWAY\*\*\s*(.*?)(?:\*\*OVERALL VERDICT\*\*|\Z)",
            raw_output,
            re.IGNORECASE | re.DOTALL,
        )
