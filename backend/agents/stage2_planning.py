...
def _extract_section_items(text: str, section_label: str) -> list[str]:
    ...
    # Step 3a: Prefer ### Action headings (current model format)
    heading_items = re.findall(r"^###\s+(.+)$", block, re.MULTILINE)
    if heading_items:
        filtered = [
            h.strip().rstrip(":").strip()
            for h in heading_items
            if not any(sk in h.lower() for sk in _SKIP_LABELS) and len(h.strip()) > 5
        ]
        if filtered:
            return filtered

    # Step 3b: Prefer bold action titles (**Action N: description**)
    ...
    # Step 3c: Fallback — plain bullet/numbered items, skip sub-property lines
    _SKIP_RE = re.compile(
